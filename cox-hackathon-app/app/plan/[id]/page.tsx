...
export default function PlanPage() {
  ...
  if (!plan) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-canopy-bg">
        <div className="text-center">
          <p className="text-canopy-muted">Plan not found.</p>
          <button
            onClick={() => router.push('/dashboard')}
            className="mt-4 text-sm text-canopy-green underline"
          >
            Back to dashboard
          </button>
        </div>
  ...
  return (
    <div className="flex h-screen w-screen flex-col overflow-hidden bg-canopy-bg">
      {/* Top bar */}
      <header className="flex shrink-0 items-center gap-3 border-b border-canopy-border bg-canopy-surface px-5 py-3">
        <div className="flex items-center gap-2">
          <Leaf className="h-4 w-4 text-canopy-green" />
          <span className="font-mono text-[11px] uppercase tracking-widest text-canopy-green">Canopy</span>
        </div>
        <ChevronRight className="h-3.5 w-3.5 text-canopy-muted" />
        <span className="truncate text-sm font-semibold text-canopy-text">{plan.name}</span>
        <div className="ml-auto flex items-center gap-2">
          <button
            onClick={() => router.push('/dashboard')}
            aria-label="Back to dashboard"
            className="rounded-lg p-1.5 text-canopy-muted transition-colors hover:bg-canopy-bg hover:text-canopy-text"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          ...
          className="flex shrink-0 flex-col overflow-hidden border-r border-canopy-border bg-canopy-surface"
        >
          {/* Selected option card */}
          <div className="border-b border-canopy-border p-5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[11px] uppercase tracking-wide text-canopy-green">{rec.category}</p>
                <h2 className="mt-0.5 text-lg font-bold text-canopy-text">{opt.name}</h2>
          ...
          {/* Tab buttons */}
          <div className="flex shrink-0 border-b border-canopy-border">
            {([
              ...
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-1.5 border-b-2 px-3 py-3 text-xs font-medium transition-colors ${
                  tab === tid
                    ? 'border-canopy-green text-canopy-green'
                    : 'border-transparent text-canopy-muted hover:text-canopy-text'
                }`}
              >
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {tab === 'financial' && (
              <div className="flex flex-col gap-5 p-5">
                {/* Key metrics */}
                <section>
                  <p className="mb-2 text-[11px] font-mono uppercase tracking-widest text-canopy-muted">Key Metrics</p>
                ...
                <section>
                  <p className="mb-1 text-[11px] font-mono uppercase tracking-widest text-canopy-muted">
                  ...
                  <div className="h-[180px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={projections} margin={{ top: 4, right: 8, bottom: 4, left: 0 }}>
                        <CartesianGrid stroke="#1e3a22" strokeDasharray="3 3" />
                        <XAxis
                          dataKey="year"
                          tick={{ fill: '#93a99b', fontSize: 10 }}
                          tickLine={false}
                          axisLine={false}
                          label={{ value: 'Year', position: 'insideBottom', offset: -2, fill: '#93a99b', fontSize: 10 }}
                        />
                        <YAxis
                          tick={{ fill: '#93a99b', fontSize: 10 }}
                          tickLine={false}
                        ...
                        <Tooltip
                          ...
                          formatter={(v: number) => [`$${Math.round(v).toLocaleString()}`, 'Net value']}
                ...
                {/* Cost breakdown */}
                <section>
                  <p className="mb-2 text-[11px] font-mono uppercase tracking-widest text-canopy-muted">Cost Breakdown</p>
                  <div className="space-y-1.5 rounded-xl border border-canopy-border bg-canopy-bg p-4">
                ...
                {incentives.availableIncentives.length > 0 && (
                  <section>
                    <p className="mb-2 text-[11px] font-mono uppercase tracking-widest text-canopy-muted">Available Incentives</p>
                    <div className="space-y-2 rounded-xl border border-canopy-border bg-canopy-bg p-4">
                      ...
                      {incentives.estimatedTotalIncentiveValue != null && (
                        <div className="flex items-center justify-between border-t border-canopy-border pt-2 text-sm">
                          <span className="font-medium text-canopy-text">Total incentives</span>
            ...
            {tab === 'checklist' && (
              <div className="flex flex-col gap-4 p-5">
                {checkItems.length === 0 ? (
                  <p className="text-sm text-canopy-muted">No checklist available for this option.</p>
                ) : (
                  <>
                    {/* Progress */}
                    <div>
                      <div className="mb-1.5 flex items-center justify-between text-sm">
                        <span className="font-medium text-canopy-text">{doneCount} of {checkItems.length} completed</span>
                      ...
                      <div className="h-2 w-full overflow-hidden rounded-full bg-canopy-border">
                        <div
                          className="h-full rounded-full bg-canopy-green transition-all duration-300"
                        ...
                        return (
                          <button
                            ...
                            className={`flex w-full items-start gap-3 rounded-xl border p-4 text-left transition-colors ${
                                ...
                                ? 'border-canopy-green/40 bg-canopy-green/5'
                          ...
                          >
                            <span
                              className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border-2 transition-colors ${
                                  ...
                                  ? 'border-canopy-green bg-canopy-green'
                                  : 'border-canopy-border'
                              }`}
                            >
                              {done && <Check className="h-3 w-3 text-canopy-bg" />}
                            </span>
                            <div className="min-w-0">
                              <p className={`text-sm font-medium leading-snug ${done ? 'text-canopy-muted line-through' : 'text-canopy-text'}`}>
                                {item.task}
                              </p>
                              <p className="mt-1 text-xs leading-relaxed text-canopy-muted">{item.description}</p>
                            </div>
                          </button>
                        )
                      })}
            ...
            {tab === 'environmental' && (
              <div className="flex flex-col gap-5 p-5">
                {/* CO2 metrics */}
                <section>
                  <p className="mb-2 text-[11px] font-mono uppercase tracking-widest text-canopy-muted">Carbon Impact</p>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  {env.plainEnglishEquivalent && (
                    <div className="mt-2 flex items-start gap-1.5 rounded-xl border border-canopy-green/30 bg-canopy-green/5 p-3">
                      <Leaf className="mt-0.5 h-3.5 w-3.5 shrink-0 text-canopy-green" />
                      <p className="text-sm text-canopy-text">{env.plainEnglishEquivalent}</p>
                    </div>
                  )}
                </section>

                {/* Other environmental benefits */}
                <section className="space-y-2 text-xs text-canopy-muted">
                  {env.stormwaterBenefit && (
                    <p className="flex items-start gap-1.5 leading-relaxed">
                      <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-canopy-green" />
                      {env.stormwaterBenefit}
                    </p>
                  )}
                ...
                <section>
                  <p className="mb-2 text-[11px] font-mono uppercase tracking-widest text-canopy-muted">Annual Impact Equivalencies</p>
                  <div className="space-y-2 rounded-xl border border-canopy-border bg-canopy-bg p-4 text-sm">
                    {carsOffRoad && (
                      <div className="flex items-center justify-between">
                        <span className="text-canopy-muted">Cars off the road</span>
                        <span className="font-mono font-semibold text-canopy-green">{carsOffRoad}</span>
                      </div>
                    )}
                    {treesEquivalent && (
                      <div className="flex items-center justify-between">
                        <span className="text-canopy-muted">Trees planted</span>
                        <span className="font-mono font-semibold text-canopy-green">{treesEquivalent.toLocaleString()}</span>
                      </div>
                    )}
                    {homesPowered && (
                      <div className="flex items-center justify-between">
                        <span className="text-canopy-muted">Homes powered</span>
                        <span className="font-mono font-semibold text-canopy-green">{homesPowered}</span>
                      </div>
                    )}
                  </div>
                  <p className="mt-2 text-[10px] leading-relaxed text-canopy-muted">
        ...
        <div
          ...
          className="relative z-10 w-1 shrink-0 cursor-col-resize bg-canopy-border transition-colors hover:bg-canopy-green/60 active:bg-canopy-green"
        />

        {/* ── 3D scene ── */}
        <div className="relative flex-1 overflow-hidden bg-canopy-bg">
          <BuildingScene3D />
        </div>
      </div>
    </div>
